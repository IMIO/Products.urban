...
def Mailing(self):
    ...
    destinationDoc = desktop.loadComponentFromURL("private:factory/swriter", "_blank", 0, () )
    destinationDocFrame = destinationDoc.getCurrentController().getFrame()
    destinationDocDispatcher = smgr.createInstanceWithContext("com.sun.star.frame.DispatchHelper", ctx)
    ...
    for recipient in recipientslist:
        if firstItem:
            ...
            sourceDocDispatcher.executeDispatch(sourceDocFrame, ".uno:SelectAll", "", 0, ())
            sourceDocDispatcher.executeDispatch(sourceDocFrame, ".uno:Copy", "", 0, ())
            destinationDocDispatcher.executeDispatch(destinationDocFrame, ".uno:Paste", "", 0, ())
