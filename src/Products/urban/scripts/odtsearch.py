...
class SearchPODTemplates(object):
    ...
    def __init__(
        self, find_expr, filenames_expr, ignorecase=False, recursive=False, silent=False
    ):
        """ """
        ...
        self.filenames_expr = (
            type(filenames_expr) not in (list, tuple)
            and [filenames_expr]
            or filenames_expr
        )
    ...
    def run(self, find_expr=None):
        ...
        self.find_expr = (
            find_expr and re.compile(find_expr, self.flags) or self.find_expr
        )
    ...
    def find_files(self, filenames_expr, recursive=False):
        ...
        result = []
        base_paths = list(
            set([os.path.dirname(path) or "." for path in filenames_expr])
        )
    ...
    def search_XML_content(self, xml_tree, filename, findexpr):
        ...
        result.extend(
            self.search_XML_pod_zone(
                annotation_nodes, filename, "commentaire", findexpr
            )
        )
...
class SearchAndReplacePODTemplates(SearchPODTemplates):
    ...
    def __init__(
        ...
        """ """
        super(SearchAndReplacePODTemplates, self).__init__(
            find_expr, filenames_expr, ignorecase, recursive
        )
        ...
        self.backup_dir = os.path.join(
            self.tmp_dir, "{}-backup".format(str(datetime.datetime.today()))
        )
    ...
    def run(self, find_expr="", replace_expr=""):
        ...
        new_files = self.replace(
            self.find_expr, self.replace_expr, search_result, self.target_dir
        )
        self.last_result = search_result
    ...
    def replace(self, find_expr, replace_expr, search_results, target_dir):
        ...
        for filename, search_result in search_results.iteritems():
            ...
            new_filename = (
                target_dir and os.path.join(target_dir, backup_filename) or filename
            )
            new_files.append(
                self.create_new_ODF(zip_file, newcontent, new_filename, target_dir)
            )
        return new_files
    ...
    def main():
        ...
        if not arguments["replace"]:
            search = SearchPODTemplates(
                **dict([(k, v) for k, v in arguments.iteritems() if v])
            )
            result = search.run()
        else:
            replace = SearchAndReplacePODTemplates(
                **dict([(k, v) for k, v in arguments.iteritems() if v])
            )
            result = replace.run()
        return result
