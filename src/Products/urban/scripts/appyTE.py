...
__author__ = """Jean-Michel Abe <jm.abe@la-bruyere.be>, Gauthier BASTIEN <gbastien@commune.sambreville.be>, Stephan GEULETTE
                    ...
                    if self.combobox2.getText()==item.attrib["name"]:
                        for action in item:                                 
                            if action.tag=="FieldCode":
                                txtfield = document.createInstance("com.sun.star.text.TextField.Input")
                                txtfield.Content = action.text.strip()
                                txtfield.Hint = self.combobox1.getText()+'-'+self.combobox2.getText()
                            ...
                            if action.tag=="AnnotationCode":
                                ...
                                document.Text.insertTextContent(ocurs, annotation, False)
...
def appyTE(self):
    ...
    oWindow = createTopWindow("Insertion de champs urban",ctx, smgr, oAwtToolkit,100,80,400,100)
    label1 = addLabel(oWindow,smgr,ctx,"FixedText",5,15,70,28,('Name','Label'),('label',u'Objet:'))
    label2 = addLabel(oWindow,smgr,ctx,"FixedText",5,40,70,28,('Name','Label'),('label',u'Elément:'))
    ...
    combobox1.addItemListener( ListItemListener(oWindow, combobox1, combobox2, label1, objects) ) 
    button1.addActionListener( InsertButtonListener(smgr,combobox1, combobox2, desktop,objects) ) 
    i=0    
    for obj in objects:
...
def createTopWindow(sTitle, ctx, smgr, oAwtToolkit, x, y, width, height):
    ...
    oWindowDesc.Parent = desktop.getCurrentComponent().CurrentController.Frame.ContainerWindow
