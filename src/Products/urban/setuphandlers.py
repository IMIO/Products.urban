    ...
    for urban_type in URBAN_TYPES:
        ...
        if urban_type in ['UrbanCertificateOne', 'UrbanCertificateTwo', 'NotaryLetter', ]:
            ...
            if not hasattr(aq_base(configFolder), 'specificfeatures'):
                ...
                newFolder.invokeFactory("UrbanVocabularyTerm",id="situe-en-zone",title=u"Situé en Zone [...]", description="<p>est situé en [[python: object.displayValue(object.Vocabulary('folderZone')[0], object.getFolderZone())]] au plan de secteur de NAMUR adopté par Arrêté Ministériel du 14 mai 1986 et qui n'a pas cessé de produire ses effets pour le bien précité;</p>")
                newFolder.invokeFactory("UrbanVocabularyTerm",id="plan-de-secteur",title=u"Plan de secteur", description="<p>est situé - dans un périmètre ... - en [[python: object.displayValue(object.Vocabulary('folderZone')[0], object.getFolderZone())]] au projet - de révision du - de - plan de secteur de ... adopté par ... du ...;</p>")
                newFolder.invokeFactory("UrbanVocabularyTerm",id="plan-communal-ammenagement",title=u"En Plan Communal d'Aménagement", description="<p>est situé en [[python: object.displayValue(object.Vocabulary('folderZone')[0], object.getFolderZone())]] dans le périmètre du plan communal d'aménagement ... approuvé par ... du ... et qui n'a pas cessé de produire ses effets pour le bien précité;</p>")
        ...
        if field.type == 'boolean':
            ...
            if hasattr(field, 'vocabulary') and field.vocabulary:
                if type(field.vocabulary) == str:
                    return getattr(licence, field.vocabulary)()[0]
                else:
