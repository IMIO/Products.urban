...
class UrbanStatsView(BrowserView):
    ...
    def computeStatistics(self, args=None):
        ...
        for brain in brains:
            state = self.map_deposit_and_complete_state_to_in_progress(
                brain.review_state
            )
    ...
    def map_deposit_and_complete_state_to_in_progress(self, review_state):
        ...
        return (
            "in_progress" if review_state in ("complete", "deposit") else review_state
        )

    def map_in_progress_state(self, licence_states):
        """Ensure 'in_progress' covers 'complete' and 'deposit' in the filter."""
        return (
            licence_states + ["deposit", "complete"]
            if "in_progress" in licence_states
            else licence_states
        )
