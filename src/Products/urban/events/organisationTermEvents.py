...
def createLinkedOpinionRequest(term, event):
    ...
    if new_id in urban_events.objectIds():
        return
    urban_events.invokeFactory('UrbanEventType', id=new_id, title='Demande d\'avis (%s)' %term.title, activatedFields=['transmitDate', 'receiptDate', 'receivedDocumentReference', 'adviceAgreementLevel', 'externalDecision', ], deadLineDelay = 15, TALCondition = "python: here.mayAddOpinionRequestEvent('%s')" %term.getId(), podTemplates = ({'id': "urb-avis-ccatm", 'title': "Courrier de demande d'avis"},), eventTypeType = 'Products.urban.interfaces.IOpinionRequestEvent')
