...
class BaseInspection(BaseFolder, GenericLicence, Inquiry, BrowserDefaultMixin):

    security = ClassSecurityInfo()
    implements(interfaces.IBaseInspection)
    ...
    def getParcels(self):
        ...
        return super(BaseInspection, self).getParcels()
    ...
    def getOfficialParcels(self):
        ...
        return super(BaseInspection, self).getOfficialParcels()

    def getApplicants(self):
        """ """
        applicants = super(BaseInspection, self).getApplicants()
        if self.getUse_bound_licence_infos():
            bound_licences = self.getBound_licences()
            if bound_licences:
    ...
    def get_applicants_history(self):
        applicants = super(BaseInspection, self).get_applicants_history()
        if self.getUse_bound_licence_infos():
            bound_licences = self.getBound_licences()
            if bound_licences:
