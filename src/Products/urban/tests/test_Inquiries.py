...
class TestBuildLicenceInquiries(unittest.TestCase):
    ...
    def testGenericLicenceGetInquiries(self):
        ...
        #to get at least one inquiry, we have to define a investigationStart on the licence
        startDate = DateTime('01/01/2011')
        self.buildLicence.setInvestigationStart(startDate)
        self.assertEqual(self.buildLicence.getInquiries(), [self.buildLicence,])
        #we can add extra inquiries by adding "Inquiry" objects
        inquiry = self._addInquiry()
        self.assertEqual(self.buildLicence.getInquiries(), [self.buildLicence, inquiry])
        #special case, if we remove the defined investigationStart date on the licence
    ...
    def testGenericLicenceGetUrbanEventInquiries(self):
        ...
        self.assertRaises(Unauthorized, createObject, 'UrbanEventInquiry', 'enquete-publique', self.buildLicence)
        #define an inquiry on the licence so we can add an UrbanEventInquiry
        startDate = DateTime('01/01/2011')
        self.buildLicence.setInvestigationStart(startDate)
        createObject('UrbanEventInquiry', 'enquete-publique', self.buildLicence)
        self.assertEquals(len(self.buildLicence.getUrbanEventInquiries()), 1)
        #we can not add a second urbanEventInquiry if only one inquiry is defined
        self.assertRaises(Unauthorized, createObject, 'UrbanEventInquiry', 'enquete-publique', self.buildLicence)
    ...
    def testUrbanEventInquiryGetLinkedInquiry(self):
        """
          ...
          There is a "1 to 1" link between an Inquiry and an UrbanEventInquiry (if exists)
          An Inquiry can exist alone but an UrbanEventInquiry must be linekd to an existing Inquiry
        """
        #define an inquiry on the licence so we can add an UrbanEventInquiry
        startDate = DateTime('01/01/2011')
        self.buildLicence.setInvestigationStart(startDate)
        ...
        #define a second Inquiry so we will be able to add a second UrbanEventInquiry
        inquiry2 = self._addInquiry()
        urbanEventInquiry2 = createObject('UrbanEventInquiry', 'enquete-publique', self.buildLicence)
    ...
    def testCanNotDeleteFirstInquiryIfLinkedToAnUrbanEventInquiryValidator(self):
        """
          We can not remove an existing Inquiry if it is linked to an UrbanEventInquiry
          This is done in the user interface by the investigationStart validator
        """
        #by default, startDate is None
        startDate = None
        #no message is returned by the validator if no UrbanEventInquiry is defined
        #no matter what is passed to the validator : None here...
        self.assertEquals(self.buildLicence.validate_investigationStart(startDate), None)
        startDate = DateTime('01/01/2011')
        #... or a date here
        self.buildLicence.setInvestigationStart(startDate)
        self.assertEquals(self.buildLicence.validate_investigationStart(startDate), None)
        ...
        createObject('UrbanEventInquiry', 'enquete-publique', self.buildLicence)
        #now that an UrbanEventInquiry is linked to the inquiry, we can not remove the defined date
        startDate = None
        self.assertNotEquals(self.buildLicence.validate_investigationStart(startDate), None)
    ...
    def testCanNotDeleteNextInquiriesIfLinked(self):
        """
          ...
          If we have several inquiries, the behaviour is the same : we can not remove an
          Inquiry that is already linked an UrbanEventInquiry
        ...
        self.assertRaises(BeforeDeleteException, self.buildLicence.manage_delObjects, inquiry2.id)
    ...
    def testCanNotDeleteUrbanEventInquiryIfNotTheLast(self):
        """
          To keep a logical behaviour, we can only remove the last UrbanEventInquiry
        """
        ...
        urbanEventInquiry2 = createObject('UrbanEventInquiry', 'enquete-publique', self.buildLicence)
        self._addInquiry()
        urbanEventInquiry3 = createObject('UrbanEventInquiry', 'enquete-publique', self.buildLicence)
        #we cannot not remove an UrbanEventInquiry if it is not the last
        self.assertRaises(BeforeDeleteException, self.buildLicence.manage_delObjects, urbanEventInquiry2.id)
        self.assertRaises(BeforeDeleteException, self.buildLicence.manage_delObjects, urbanEventInquiry1.id)
