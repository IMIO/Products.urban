...
class TestSearchStreetsForm(unittest.TestCase):
    ...
    def testSearchStreetsWithoutContent(self):
        """perform a streets search without content, thus no result must be returned
        """
        self.browser.open(self.urban.absolute_url()+'/urban_searchbystreet')
        self.browser.getControl("Rechercher").click()
        self.assertFalse("Résultats de la recherche courante" in self.browser.contents)

    def testSearchStreetsBuildlicenceWithtoutStreet(self):
        """create a buildlience content without a street and check that this content is found
           if search is performed without indicating a particular street
        """
        #create a buildlicence content
        self.buildLicences = self.urban.buildlicences
        self.buildLicences.invokeFactory('BuildLicence', 'buildlicence1', title=u"test_buildlicence")
        import transaction
        transaction.commit()
        self.browser.open(self.urban.absolute_url()+'/urban_searchbystreet')
        self.browser.getControl("Rechercher").click()
        #check that buildlicence content is found
        self.assertTrue("Résultats de la recherche courante" in self.browser.contents)

    def testSearchStreetsNotBuildlicenceWithtoutStreet(self):
        """create a buildlience content without a street and check that this content is not found
           if search is performed without indicating a particular street and by deselecting buildlicence type
        """
        #create a buildlicence content
        self.buildLicences = self.urban.buildlicences
        self.buildLicences.invokeFactory('BuildLicence', 'buildlicence1', title=u"test_buildlicence")
        ...
        self.browser.getControl("Rechercher").click()
        #check that non content is found
        self.assertFalse("Résultats de la recherche courante" in self.browser.contents)

    def testSearchStreetsDeclarationWithtoutStreet(self):
        """create a buildlience content without a street and check that this content is found
           if search is performed without indicating a particular street
        """
        #create a declaration content
        self.declarations = self.urban.declarations
        self.declarations.invokeFactory('Declaration', 'declaration1', title=u"test_declaration")
        import transaction
        transaction.commit()
        self.browser.open(self.urban.absolute_url()+'/urban_searchbystreet')
        self.browser.getControl("Rechercher").click()
        #check that declaration content is found
        self.assertTrue("Résultats de la recherche courante" in self.browser.contents)

    def testSearchStreetsNotDeclarationWithtoutStreet(self):
        """create a buildlience content without a street and check that this content is not found
           if search is performed without indicating a particular street and by deselecting buildlicence type
        """
        #create a declaration content
    ...
    def XXXtestSearchStreetsBuildlicenceWithStreet(self):
        """create a buildlicence content with a street location. Check that this content is found
        ...
        transaction.commit()
	self.browser.open(self.urban.absolute_url()+'/urban_searchbystreet')
        self.browser.getControl(name = 'form.streetSearch.to').value= [self.street.UID(),]
        self.browser.getControl("Rechercher").click()
        self.assertTrue("Résultats de la recherche courante" in self.browser.contents)
