...
class UrbanDocGenerationLicenceHelperView(UrbanDocGenerationHelperView):
    ...
    def get_related_licences(
        self,
        licence_types=[],
        decision_limit_date=None,
        licence_state=None,
        with_historic=False,
    ):
        related_licences = [
            licence.getObject()
            for licence in self.get_related_licences_of_parcel(
                licence_types, with_historic
            )
        ]
        ...
        if decision_limit_date:
            ...
            for licence in related_licences:
                delivered = ""
                if IBaseBuildLicence.providedBy(
                    licence
                ) or IUrbanCertificateBase.providedBy(licence):
                ...
                if delivered and (
                    (delivered.getDecisionDate() or delivered.getEventDate())
                    >= decision_limit_date
                ):
    ...
    def get_related_licences_of_parcel(self, licence_types=[], with_historic=False):
        ...
        for parcel in parcels:
            for brain in parcel.getRelatedLicences(
                licence_type=licence_types, with_historic=with_historic
            ):
        ...
        for brain in self.get_related_licences_of_parcel(licence_types):
            ...
            if (
                ...
                if delivered.getDecision() == "favorable":
                    ...
                    licences.append(licence)
            elif not delivered and api.content.get_state(licence) in [
                "accepted",
                "ended",
            ]:
                licences.append(licence)
    ...
    def get_parcels(self, with_commas=False, with_division=True):
        ...
        for gp in enumerate(list_grouped_parcels):
            ...
            if with_division:
                if with_commas:
                    result += u"{}, section {}, ".format(
                        divisionAlternativeName, section
                    )
                else:
        ...
        for opinionRequestEventType in opinion_cfgs:
            ...
            if opinionRequestEventType.id in self.getSolicitOpinions():
                ...
                for urbanEventOpinionRequest in all_opinion_request_events:
                    if (
                        urbanEventOpinionRequest.getUrbaneventtypes()
