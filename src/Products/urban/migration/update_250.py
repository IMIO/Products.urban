...
def macro_end_condition_object_converter(value):
    return macro_condition_converter(value)


@adapter(MacroCreationConditionObject)
@implementer(IJsonCompatible)
def macro_creation_condition_object_converter(value):
    return macro_condition_converter(value)


@adapter(MacroRecurrenceConditionObject)
@implementer(IJsonCompatible)
def macro_recurence_condition_object_converter(value):
    return macro_condition_converter(value)
...
def _replace_object(obj, new_type, condition=None):
    portal = api.portal.get()
    ...
    if "calculation_delay" in old_obj_data and old_obj_data["calculation_delay"]:
        new_obj.calculation_delay = [
            item['token'] for item in old_obj_data["calculation_delay"]
        ]
    if "marker_interfaces" in old_obj_data and old_obj_data["marker_interfaces"]:
        new_obj.marker_interfaces = set(
            [item['token'] for item in old_obj_data["marker_interfaces"]]
        )
    if "creation_state" in old_obj_data and old_obj_data["creation_state"]:
        new_obj.creation_state = set(
            [item['token'] for item in old_obj_data["creation_state"]]
        )
    if "starting_states" in old_obj_data and old_obj_data["starting_states"]:
        new_obj.starting_states = set(
            [item['token'] for item in old_obj_data["starting_states"]]
        )
    if "ending_states" in old_obj_data and old_obj_data["ending_states"]:
        new_obj.ending_states = set(
            [item['token'] for item in old_obj_data["ending_states"]]
        )
    if "freeze_states" in old_obj_data and old_obj_data["freeze_states"]:
        new_obj.freeze_states = set(
            [item['token'] for item in old_obj_data["freeze_states"]]
        )
    if "thaw_states" in old_obj_data and old_obj_data["thaw_states"]:
        new_obj.thaw_states = set(
            [item['token'] for item in old_obj_data["thaw_states"]]
        )
    if "recurrence_states" in old_obj_data and old_obj_data["recurrence_states"]:
        new_obj.recurrence_states = set(
            [item['token'] for item in old_obj_data["recurrence_states"]]
        )

    if "creation_conditions" in old_obj_data and old_obj_data["creation_conditions"]:
        new_obj.creation_conditions = set([
            MacroCreationConditionObject(
                condition=item["condition"],
                operator=item["operator"],
                display_status=item["display_status"]
            )
            for item in old_obj_data["creation_conditions"]
        ])
    if "start_conditions" in old_obj_data and old_obj_data["start_conditions"]:
        new_obj.start_conditions = set([
            MacroStartConditionObject(
                condition=item["condition"],
                operator=item["operator"],
                display_status=item["display_status"]
            )
            for item in old_obj_data["start_conditions"]
        ])
    if "end_conditions" in old_obj_data and old_obj_data["end_conditions"]:
        new_obj.end_conditions = set([
            MacroEndConditionObject(
                condition=item["condition"],
                operator=item["operator"],
                display_status=item["display_status"]
            )
            for item in old_obj_data["end_conditions"]
        ])
    if "freeze_conditions" in old_obj_data and old_obj_data["freeze_conditions"]:
        new_obj.freeze_conditions = set([
            MacroFreezeConditionObject(
                condition=item["condition"],
                operator=item["operator"],
                display_status=item["display_status"]
            )
            for item in old_obj_data["freeze_conditions"]
        ])
    if "thaw_conditions" in old_obj_data and old_obj_data["thaw_conditions"]:
        new_obj.thaw_conditions = set([
            MacroThawConditionObject(
                condition=item["condition"],
                operator=item["operator"],
                display_status=item["display_status"]
            )
            for item in old_obj_data["thaw_conditions"]
        ])
    if "recurrence_conditions" in old_obj_data and old_obj_data["recurrence_conditions"]:
        new_obj.recurrence_conditions = set([
            MacroRecurrenceConditionObject(
                condition=item["condition"],
                operator=item["operator"],
                display_status=item["display_status"]
            )
            for item in old_obj_data["recurrence_conditions"]
        ])
...
def fix_config_wrong_class(context):
    ...
    for licence_config in portal_urban.objectValues('LicenceConfig'):
        ...
        if schedule_cfg and hasattr(schedule_cfg, 'announcement-preparation'):
            ...
            _replace_object(announcement_prep_task, "MacroTaskConfig")
